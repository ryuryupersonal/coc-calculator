...
class DefenseTableSource extends DataGridSource {
  ...
  : _source = source, _quakeLevel = quakeLevel,
        ...
        if (dataCell.columnName == 'th') {
          ...
          return Container(
            ...
            child: Text(
              th,
              style: TextStyle(
                color: isMax? Colors.indigoAccent : null
              ),
          ...
          final defense = _display(dataCell.value.defense, dataCell.value.level);
  ...
  List<DataGridRow> _makeRows(List<DefenseModel> source) => source.map<DataGridRow>(
    (data) => DataGridRow(
      cells: [
